  ...
  const aiResponsibilities = async (i) => {
    ...
    try {
      let text = await callClaude(
        `Write exactly 4 job responsibilities for a ${e.role || "professional"} at ${e.company || "a company"}. Each on a new line starting with a dash and action verb. No headings, no markdown, no preamble. Just the 4 lines.`
      );
      text = text.replace(/^#.*$/gm, "").trim();
      const exp = [...cv.experience];
      exp[i] = { ...exp[i], responsibilities: text };
      setCV(p => ({ ...p, experience: exp }));
    } catch (err) { alert("AI error: " + err.message); }
    setLoading(l => ({ ...l, [key]: false }));
  };
      ...
      <div className="cv-form__section">
        ...
        <div className="cv-form__grid">
          ...
          <div className="cv-form__field">
            ...
            <Input value={cv.religion || ""} onChange={upd("religion")} onBlur={blur("religion")} placeholder="e.g. Christianity, Islam" />
          </div>
        </div>
        ...
        <div className="cv-form__field">
          ...
          <AiBtn
            ...
            onClick={() => aiSuggest("summary",
              `Write exactly 2 sentences summarising ${cv.name || "this professional"} as a ${cv.jobTitle || "professional"}. Sentence 1: who they are and their experience. Sentence 2: their key strength. Plain text only, no heading, start immediately with the name or role.`
            )}
          />
        </div>
        <div className="cv-form__field">
      ...
      <div className="cv-form__section">
        ...
        <div className="cv-form__field">
          ...
          <AiBtn
            ...
            onClick={() => aiSuggest("skills",
              `List exactly 8 skills for a ${cv.jobTitle || "professional"}. Return as comma-separated values only. No headings, no numbers, no markdown. Just the list.`
            )}
          />
        </div>
        ...
        <div className="cv-form__field">
          ...
          <AiBtn
            ...
            onClick={() => aiSuggest("languages",
              `Suggest languages and proficiency levels for a ${cv.jobTitle || "professional"} in Nigeria. Return as comma-separated list only. Example: English (Fluent), Yoruba (Native). No headings, no markdown.`
            )}
          />
        </div>
        ...
        <div className="cv-form__field">
          ...
          <AiBtn loading={loading.certs} onClick={aiSuggestCertifications} />
        </div>
      </div>
    );
