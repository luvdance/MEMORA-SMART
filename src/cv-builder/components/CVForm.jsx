...
export default function CVForm({ cv, setCV, template, step, setStep, tab }) {
  ...
  const addExp = () => setCV(p => ({ ...p, experience: [...p.experience, { company: "", role: "", start: "", end: "", current: false, responsibilities: "" }] }));
  const addEdu = () => setCV(p => ({ ...p, education: [...p.education, { school: "", degree: "", field: "", start: "", end: "" }] }));
  ...
  const renderStep = () => {
    if (step === 0) return (
      <div className="cv-form__section">
        ...
        {[["Full Name","name"],["Job Title","jobTitle"],["Email","email"],["Phone","phone"],["Address","address"],["LinkedIn","linkedin"],["Website","website"]].map(([lbl, key]) => (
          <div className="cv-form__field" key={key}>
            <Label>{lbl}</Label>
            <Input value={cv[key]} onChange={upd(key)} placeholder={lbl} />
          </div>
        ))}
      </div>
    );
    ...
    if (step === 2) return (
      <div className="cv-form__section">
        ...
        <div className="cv-form__field">
          ...
          <Textarea value={cv.summary} onChange={upd("summary")} placeholder="A brief professional summary..." rows={4} />
        ...
        <div className="cv-form__field">
          ...
          <Textarea value={cv.objective} onChange={upd("objective")} placeholder="Your career objective..." rows={3} />
        ...
        {cv.experience.map((e, i) => (
          <div key={i} className="cv-form__card">
            ...
            {[["Company","company"],["Job Title / Role","role"]].map(([lbl, key]) => (
              <div className="cv-form__field" key={key}>
                <Label>{lbl}</Label>
                <Input value={e[key]} onChange={updExp(i, key)} placeholder={lbl} />
              </div>
            ))}
            <div className="cv-form__grid">
              <div><Label>Start Date</Label><Input value={e.start} onChange={updExp(i,"start")} placeholder="e.g. Jan 2020" /></div>
              <div><Label>End Date</Label><Input value={e.end} onChange={updExp(i,"end")} placeholder="e.g. Dec 2022" /></div>
            </div>
            ...
            <div className="cv-form__field">
              ...
              <Textarea value={e.responsibilities} onChange={updExp(i,"responsibilities")} placeholder="Describe your duties and achievements..." rows={4} />
      ...
      <div className="cv-form__section">
        ...
        {cv.education.map((e, i) => (
          <div key={i} className="cv-form__card">
            ...
            <div className="cv-form__grid">
              <div><Label>Start Year</Label><Input value={e.start} onChange={updEdu(i,"start")} placeholder="e.g. 2016" /></div>
              <div><Label>End Year</Label><Input value={e.end} onChange={updEdu(i,"end")} placeholder="e.g. 2020" /></div>
            </div>
          </div>
        ))}
    ...
    if (step === 5) return (
      <div className="cv-form__section">
        ...
        <div className="cv-form__field">
          ...
          <Textarea value={cv.skills} onChange={upd("skills")} placeholder="e.g. Project Management, Excel, Leadership..." rows={3} />
        ...
        <div className="cv-form__field">
          ...
          <Input value={cv.languages} onChange={upd("languages")} placeholder="e.g. English (Fluent), French (Intermediate)" />
        </div>
        <div className="cv-form__field">
          <Label>Certifications</Label>
          <Textarea value={cv.certifications} onChange={upd("certifications")} placeholder="e.g. PMP, AWS Certified..." rows={3} />
        </div>
      </div>
    );
