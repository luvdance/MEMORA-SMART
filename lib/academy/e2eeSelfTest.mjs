...
/* ── Messages this device cannot open are not shown ──────────────────────
 * This went through three versions. First, every undecryptable message
 * rendered its own line — "Encrypted with a key this browser does not have" —
 * once per message, which repeated, described our storage to somebody who
 * could not act on it, and blamed the wrong side, since the usual cause is the
 * OTHER person reinstalling. Then one grouped explanation per run, which was
 * accurate and still a wall of text in the middle of a conversation.
 *
 * Now they are simply not rendered. A message that cannot be opened is not
 * something a reader can act on, and the panel already says the thread is
 * end-to-end encrypted — which is the whole reason older messages sometimes do
 * not appear.
 */
section("Unreadable messages are not rendered");

const paneNow = readFileSync(
  new URL("../../src/academy/components/SocialPane.jsx", import.meta.url), "utf8");

ok("the per-message warning is gone",
  !/Encrypted with a key this browser does not have/.test(paneNow));
ok("and so is the grouped explanation that replaced it",
  !/ac-sp__locked|UnreadableRun|groupThread/.test(paneNow));
ok("undecryptable messages are filtered out rather than rendered",
  /m\.text !== null/.test(paneNow));
ok("the filter is null while loading, so the empty state does not flash",
  /messages === null \? null :/.test(paneNow));
ok("the empty state follows what is readable, not what was fetched",
  /readable\?\.length === 0/.test(paneNow));

/* The encryption notice is a label now, not an explanation of our storage. */
ok("the panel says end-to-end encrypted and nothing about ciphertext",
  /End-to-end encrypted/.test(paneNow) &&
  !/ciphertext|we store only/.test(paneNow));
...
/* ── The published key document must match what the rules allow ──────────
 * This is the test that was missing. Recording rotation looked harmless, so
 * `rotatedAt: serverTimestamp()` went into the write. firestore.rules pins the
 * document to exactly four keys with hasOnly(), and merge:true does not exempt
 * it — the rule is evaluated against the POST-merge document, so a fifth field
 * fails the ENTIRE write. The key then never published, and the learner was
 * told nobody could message them. A permission rule and the code that writes
 * through it are one unit; asserting the shape in one place and not the other
 * is how they drift. So the allowlist is read out of the rules file and
 * compared against the object the code actually writes.
 */
const rulesSrc = readFileSync(
  new URL("../../firestore.rules", import.meta.url), "utf8");

/* Slice from the match line to the rule's closing brace. Start PAST the
   "{userId}" wildcard, or the block ends before it begins. */
const ruleHead = rulesSrc.indexOf("match /publicKeys/{userId}");
const keyRule = rulesSrc.slice(
  ruleHead, rulesSrc.indexOf("allow delete", ruleHead));
const allowed = [...keyRule.matchAll(/'([a-zA-Z]+)'/g)].map((m) => m[1]);

ok("firestore.rules still pins the public key document to a fixed key set",
  allowed.length > 0 && /hasOnly/.test(keyRule), allowed.join(", "));

/* The literal passed to setDoc — the one inside publishPublicKey, not the
   first setDoc in a file that has several. */
const fn = socialNow.slice(socialNow.indexOf("export async function publishPublicKey"));
const writeCall = fn.slice(fn.indexOf("await setDoc("));
const written = [...writeCall.slice(0, writeCall.indexOf("{ merge: true }"))
  .matchAll(/^\s+([a-zA-Z]+)[,:]/gm)].map((m) => m[1]);

ok("the code writes at least the fields the rule requires",
  written.length >= 3, written.join(", "));

const disallowed = written.filter((k) => !allowed.includes(k));
ok("and writes NO field the rule would reject, which voids the whole write",
  disallowed.length === 0, `rejected by rules: ${disallowed.join(", ")}`);

ok("rotation is returned to the caller, not persisted into that document",
  !/rotatedAt/.test(socialNow) && /return \{ pair, publicKey, rotated \}/.test(socialNow));
