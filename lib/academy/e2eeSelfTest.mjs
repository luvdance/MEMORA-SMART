...
ok("presence omits fields it does not have instead of nulling them",
  /if \(student\) \{\s*row\.level/.test(socialSrc));
...
// Called in load(), in the toggle, and now on a timer.
const activeCalls = (pane.match(/getActiveLearners\(/g) || []).length;
ok("the active list is fetched more than once", activeCalls >= 2,
  `${activeCalls} call sites`);
ok("there is a timer that refreshes it",
  /setInterval\(refresh, 30 \* 1000\)/.test(pane));
ok("the heartbeat carries the student record",
  /announcePresence\(user, \{ student: meRef\.current, lessonTitle \}\)/.test(pane));
ok("a failed poll keeps the last known list rather than showing nobody",
  /\/\/ Leave the last known list up/.test(pane));
