...
ok("becoming visible announces again",
  /announce\(\);\s*if \(!interval\)/.test(hb));
...
// Staleness is re-checked on read, because the query's cutoff is fixed when
// the subscription is created.
ok("rows are re-filtered against the current clock",
  /const freshAfter = Date\.now\(\) - PRESENCE_TTL_MS;/.test(socialCode));
ok("a row whose timestamp the server has not yet set is kept",
  /seen === undefined \|\| seen === null \|\| seen >= freshAfter/.test(socialCode));
